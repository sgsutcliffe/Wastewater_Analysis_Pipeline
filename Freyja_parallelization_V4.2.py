...
def run_core_iPMVC(wp_path,current_sample, nb_t_profiling, smpl_path, usherbarcodes, output_folder):
	...
	os.system("bwa mem {3}Homo_sapiens.GRCh38.fa {0}{1}_R1_001_trimmed_1.fastq.gz {0}{1}_R2_001_trimmed_2.fastq.gz -t {2} > {0}{1}.sam".format(output_folder, current_sample, nb_t_profiling, wp_path))
	#Replace with kraken
	os.system("samtools view -bS {0}{1}.sam > {0}{1}.bam && rm {0}{1}.sam ".format(output_folder, current_sample))
	...
	os.system("samtools fastq -@ {2} {0}{1}_human_decon_sorted.bam -1 {0}{1}_R1_001_decon_1.fastq.gz -2 {0}{1}_R2_001_decon_2.fastq.gz".format(output_folder, current_sample, nb_t_profiling))
		
	#Align reads to reference genome 
	os.system("bwa mem {0}MN908947_3.fa {3}{1}_R1_001_decon_1.fastq.gz {3}{1}_R2_001_decon_2.fastq.gz -t {2} > {3}{1}_preprocessed.sam".format(wp_path, current_sample, nb_t_profiling, output_folder))
	...
	os.system("samtools index {0}{1}_ivartrim_sorted.bam".format(output_folder, current_sample))
...
if __name__ == "__main__":
	...
	#Will check if user wants to update the barcodes either with 1) freyja update 2) modified freyja update 3) no-update
	usherbarcodes = sys.argv[5]
	today = date.today()
	#ushebarcode dates are always one day old
	down_date = today - timedelta(days = 1)
	down_date =  "usher_barcodes_{0}.csv".format(down_date)
	if '-update' in sys.argv[1:]:
		print('Will update the barcodes using modified freyja update script')
		os.system("python3 {0}/usher_update_V2.py {0}".format(workspace_path))
		usherbarcodes = down_date
	elif '-fupdate' in sys.argv[1:]:
		print('Will update the barcodes using freyja update')
		#os.system("freyja update --outdir {0}/{1}} {0}".format(workspace_path, down_date))
		usherbarcodes = down_date
	else:
		print("No usher barcode update requested and will use {0}".format(usherbarcodes))
		if usherbarcodes != down_date:
			print("Warning: Not using the most up-to-date Usherbarcordes")
